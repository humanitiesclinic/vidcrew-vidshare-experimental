...
onMounted(() => {
  ...
  if (uniqueInstances.value.length > 0) {
    const firstInstance = uniqueInstances.value[0];
    if (firstInstance != null) {
      currentInstance.value = firstInstance;
    }
...
const scrollToNextCard = (delta: number) => {
  ...
  if (nextIdx >= 0 && nextIdx < notes.length) {
    ...
    if (nextNote) {
      currentCardId.value = nextNote.id;
      scrollToCard(delta);
    }
  }
};
