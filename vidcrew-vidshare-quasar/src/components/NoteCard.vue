  ...
  if (cardContainer.value) {
    intersectionObserver = new IntersectionObserver(
      entries => {
        ...
        if (!entry || !videoPlayer.value) return;
...
const startEditField = (field: string) => {
  ...
  const value = (props.note as any)[field];
...
const saveField = (field: string) => {
  ...
  if (field === 'startTime' || field === 'endTime' || field === 'instance') {
    ...
    if (!isNaN(numValue)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (props.note as any)[field] = numValue;
    }
  } else {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (props.note as any)[field] = editingValue.value;
  }
